...
def optimize_and_regenerate_udt(udt_definition: dict, all_udts: dict = None,
    ...
    if not tags or optimizable_count == 0:
        ...
        return {
            "success":             False,
            "error":               (
                f"No optimisable members found for '{udt_name}'. "
                f"All {len(skipped_members_sorted)} member(s) reference unknown types "
                f"({', '.join(skipped_types_sorted)}). "
                f"These types are likely defined in the full compiled program L5X."
            ),
    ...
    if result.get("success"):
        ...
        sorted_tags = sorted(
            cleaned_for_sort,
            key=lambda x: _sort_key(x, udt_size_registry, aoi_registry)
        )
